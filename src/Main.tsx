...
const Map: FunctionComponent = () => {
  ...
  const onMapInitialized = (
    ...
    void initial_funcs.setPolyline('polyline1', {
  ...
  const addCluster = (
    ...
    for (let i = 0; i < number_of_markers; i++) {
      ...
      promises.push(
        funcs.setMarker('cluster_' + i.toString(), { default: marker_opts }),
      );
    }
    return Promise.all(promises).then((markers) => {
      void funcs.setClusterer(cluster_options).then((clusterer) => {
  ...
  const animateClusterMarkers = (step = 0): void => {
    for (let i = 0; i < cluster_markers.length; i++) {
      ...
      void marker.setOptions({ default: opts });
    }
    ...
    <div>
      ...
      <button
        onClick={() => {
          ...
          // lng: 14.50567, lat: 56.75918
          void addCluster(funcs, 1000, 56, 57, 14, 15).then((markers) => {
